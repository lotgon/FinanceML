...
# Analysing adjusted_value_m2 series for `r targetName`
...
# Analysing `r targetName` adjusted_value_m2 (adjusted for inflation) ~ diffM200(trend) series for
...
for(curr_id in unique(data$series_id)){
  c<-ccf(data[series_id==targetName, adjusted_value_m2], data[series_id==curr_id, x], lag.max=1000, na.action=na.pass, main=curr_id, plot=FALSE)
  if( max(abs(c$acf)) > 0.2)
    ccf(data[series_id==targetName, adjusted_value_m2], data[series_id==curr_id,x], lag.max=1000, na.action=na.pass, main=curr_id)
}
```
