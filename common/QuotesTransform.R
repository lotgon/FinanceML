...
CalculateLastMax <- function(series, threshold=0.9){
  #in parameters
  #quotes as data.table[series_id, value] ordered by time
  for(sym in series[,unique(series_id)])
    CalculateLastMaxPerSeriesId(series, sym, threshold)
  series
}
CalculateLastMaxPerSeriesId <- function(quotes, chosen_series_id, threshold){
  #in parameters
  #quotes as data.table[series_id, value] ordered by time
  cPrevMax <- numeric(quotes[series_id==chosen_series_id, .N])
  i <- 0;prevMax_ <- NA;currentMax <- 0;isTrendUp <- TRUE
  for(price in quotes[series_id==chosen_series_id,value]){
    i<-i+1
    if(!is.na(price)){      
      if( price > currentMax){
        currentMax <- price
