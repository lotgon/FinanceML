...
xau <- rbind(xau, tts_GetDailyData("XAUUSD", xau[.N, date+1], observation_end))
xbr <- localStorage_getSeries("XBRUSD", observation_start, observation_end)
xbr <- rbind(xbr, tts_GetDailyData("XBRUSD", xbr[.N, date+1], observation_end))
btcusd <- tts_GetDailyData("BTCUSD", observation_start, observation_end)
tltetf <- tts_GetDailyData("TLT", observation_start, observation_end)

data <- rbind(sp500, xau, xbr, ndx, btcusd, tltetf)
data[,type:="Price/USD"]
```
...
junkBond <- fred_getSeries("BAMLH0A0HYM2EY", observation_start, observation_end)


econ_parameters <- rbind(unemployment_rate, fed_ii, T10Y2Y, inflation_rate, bond10, junkBond)
...
m2[,date:=as.Date(date)]
#stopifnot( data[series_id=="^GSPC", date] == m2$date )
data <- merge(data, m2[, .(date, value)], by="date", all.x = TRUE, all.y = FALSE, suffixes = c("", ".y"))
setnames(data, "value.y", "m2")
data[, adjusted_value_m2:=value/m2*data[.N, m2], by=series_id]
ggplot(data[series_id=="^GSPC"])  + geom_line(aes(x=date, y=adjusted_value_m2), color='blue')+ geom_line(aes(x=date, y=value), color='green')
...
data <- TranformToDropdawnData(data, columnName = "value")#columnName = "adjusted_value_m2")
#calculate growthRate and Growth Index
#drawdown shows only decline, to see the full picture, use growthIndex. 
TranformTo_LastMax(data, columnName = "value")#columnName = "adjusted_value_m2")
data[,`:=`(maxValue=NULL, dropdawn =NULL, dropdawnMVA =NULL, prevMax =NULL)]
#Sharp decline on the chart, shows drawdown 10% of latest maximum
